...
func getStartingState(reader io.Reader) CubeList {
	...
	for scanner.Scan() {
		...
		for y := 0; y < len(line); y++ {
			...
			if state == active {
				startingState.AddCubeWithCoordinates(x, y, 0, 0, active)
			}
		}
...
// Format for position: x,y,z,w
type CubeList map[string]rune

func (c CubeList) AddCubeWithCoordinates(x, y, z, w int, state rune) {
	c[getKey(x, y, z, w)] = state
}
...
func (c CubeList) getNewStateForActiveCube(key string) (newState rune, inactiveNeighbours []string) {
	...
	newState = getNewState(active, 80-len(inactiveNeighbours))
...
func getNeighboursKeys(key string) (neighboursKeys []string) {
	baseX, baseY, baseZ, baseW := getCoordinates(key)

	for x := baseX - 1; x <= baseX+1; x++ {
		for y := baseY - 1; y <= baseY+1; y++ {
			for z := baseZ - 1; z <= baseZ+1; z++ {
				for w := baseW - 1; w <= baseW+1; w++ {
					if x != baseX || y != baseY || z != baseZ || w != baseW {
						neighboursKeys = append(neighboursKeys, getKey(x, y, z, w))
					}
				}
			}
		}
...
func getKey(x, y, z, w int) string {
	return fmt.Sprintf("%d,%d,%d,%d", x, y, z, w)
}

func getCoordinates(key string) (x, y, z, w int) {
	...
	w, _ = strconv.Atoi(valuesAsString[3])
